...
void ImpressionistUI::cb_amountSlides(Fl_Widget* o, void* v)
{
	((ImpressionistUI*)(o->user_data()))->m_nAmount = int(((Fl_Slider*)o)->value());
}

void ImpressionistUI::cb_radSlides(Fl_Widget* o, void* v)
{
	((ImpressionistUI*)(o->user_data()))->m_nRad = int(((Fl_Slider*)o)->value());
}
...
int ImpressionistUI::getAmount()
{
	return m_nAmount;
}

int ImpressionistUI::getRad()
{
	return m_nRad;
}
...
void ImpressionistUI::setAmount(int amount)
{
	m_nAmount = amount;
	if (amount <= 100)
		m_BrushAmountSlider->value(m_nAmount);
}

void ImpressionistUI::setRad(int rad)
{
	m_nRad = rad;
	if (rad <= 100)
		m_BrushRadSlider->value(m_nRad);
}
...
Fl_Menu_Item ImpressionistUI::brushTypeMenu[NUM_BRUSH_TYPE+1] = {
  ...
  {"ScatteredLines",    FL_ALT + 'f', (Fl_Callback*)ImpressionistUI::cb_brushChoice, (void*)BRUSH_SCATTEREDLINES},
  {"SplitMilk",         FL_ALT + 'm', (Fl_Callback*)ImpressionistUI::cb_brushChoice, (void*)BRUSH_SPLITMILK},
  {"ReversedCircles",   FL_ALT + 'r', (Fl_Callback*)ImpressionistUI::cb_brushChoice, (void*)BRUSH_REVERSEDCIRCLES},
  {"ReversedScatteredCircles", FL_ALT + 'y', (Fl_Callback*)ImpressionistUI::cb_brushChoice, (void*)BRUSH_REVERSEDSCATTEREDCIRCLES},
  {"ReversedScatteredLines", FL_ALT + 'u', (Fl_Callback*)ImpressionistUI::cb_brushChoice, (void*)BRUSH_REVERSEDSCATTEREDLINES},
  {0}
};
...
ImpressionistUI::ImpressionistUI() {
	...
	m_brushDialog = new Fl_Window(400, 325, "Brush Dialog");
		...
		m_nAmount = 10;
		m_BrushAmountSlider = new Fl_Value_Slider(10, 200, 300, 20, "Amount");
		m_BrushAmountSlider->user_data((void*)(this));
		m_BrushAmountSlider->type(FL_HOR_NICE_SLIDER);
		m_BrushAmountSlider->labelfont(FL_COURIER);
		m_BrushAmountSlider->labelsize(12);
		m_BrushAmountSlider->minimum(1);
		m_BrushAmountSlider->maximum(100);
		m_BrushAmountSlider->step(1);
		m_BrushAmountSlider->value(m_nAmount);
		m_BrushAmountSlider->align(FL_ALIGN_RIGHT);
		m_BrushAmountSlider->callback(cb_amountSlides);

		m_nRad = 4;
		m_BrushRadSlider = new Fl_Value_Slider(10, 240, 300, 20, "Radius");
		m_BrushRadSlider->user_data((void*)(this));
		m_BrushRadSlider->type(FL_HOR_NICE_SLIDER);
		m_BrushRadSlider->labelfont(FL_COURIER);
		m_BrushRadSlider->labelsize(12);
		m_BrushRadSlider->minimum(1);
		m_BrushRadSlider->maximum(100);
		m_BrushRadSlider->step(1);
		m_BrushRadSlider->value(m_nRad);
		m_BrushRadSlider->align(FL_ALIGN_RIGHT);
		m_BrushRadSlider->callback(cb_radSlides);

		//α値スライダー
		m_nAlpha = 100;
		m_BrushAlphaSlider = new Fl_Value_Slider(10, 280, 300, 20, "Alpha");
